...
| [sae/](sae/README.md) | §9 | Phase 4 | 핵심 구현 완료(상태 머신·모델·평가·dead feature resampling, 34개 테스트) + dense/narrative-selected 두 표본으로 9개 설정 비교 — **근본 원인 특정**: 데이터셋(55농장×14일, 원시 이벤트 222,309건) 자체의 PCA 유효 차원이 384 중 29뿐이며 표본을 narrative 템플릿으로 선별해도 동일함을 직접 검증(원인 1, 확인 완료) + dict를 그 차원에 맞춰도 남는 TopK 승자독식 학습 역학(원인 2, 미해결) → §18 중단 조건 여전히 발동 |
