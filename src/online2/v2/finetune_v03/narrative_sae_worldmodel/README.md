...
| [sae/](sae/README.md) | §9 | Phase 4 | 핵심 구현 완료(상태 머신·모델·평가·dead feature resampling, 34개 테스트) + 표본 구성 9종 + encoder 6개 layer 비교 실측 — **근본 원인 특정**(데이터셋 자체의 PCA 유효 차원이 384 중 29뿐, 표본 선별 방식과 무관) **+ 개선 방향 발견**(최종 layer 대신 layer 2 사용 시 dead ratio 0.500→0.391, 재현성 확인 전) → §18 중단 조건 여전히 발동하나 다음 시도 경로가 명확해짐 |
