...
| [sae/](sae/README.md) | §9 | Phase 4 | 핵심 구현 완료(상태 머신·모델·평가·dead feature resampling, 34개 테스트) + 실제 캐시 activation으로 pilot 실행 — **결과가 §18 "feature 대부분 dead" 중단 조건에 실제로 걸림**(explained_variance=0.95지만 dead_feature_ratio=0.93), 다음 Phase 진행 전 재보정 필요 |
