"""W&B sweep agent entrypoint for Hackathon greenhouse MLM pretrain."""
...
HACKATHON_PRETRAIN_BATCH_SIZE = 16
...
def _sweep_overrides(wc) -> list[str]:
    run_id = wandb.run.id
    return [
        "experiment=sweep_hackathon_pretrain",
        ...
        f"datamodule.batch_size={HACKATHON_PRETRAIN_BATCH_SIZE}",
        f"model.hparams.batch_size={HACKATHON_PRETRAIN_BATCH_SIZE}",
        f"trainer.logger.1.name=hackathon_pretrain_sweep_{run_id}",
    ]
...
def main() -> None:
    ...
    log.info("Hackathon pretrain sweep run %s", wandb.run.id)
    train_from_cfg(cfg)
    wandb.finish()
