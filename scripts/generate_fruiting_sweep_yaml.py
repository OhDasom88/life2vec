#!/usr/bin/env python3
"""Inspect ugf1f2ld pretrain runs and optionally refresh the fruiting sweep YAML."""
...
SWEEP_YAML = PROJECT_ROOT / "conf" / "sweep" / "agri_fruiting.yaml"


def _print_runs(runs: list[dict]) -> None:
    print(f"Pretrain sweep: {PRETRAIN_SWEEP_ID}")
...
def _write_sweep_yaml(run_ids: list[str]) -> None:
    ...
    doc["parameters"]["pretrained_run_id"]["values"] = run_ids
    header = (
        "# W&B sweep: Agri fruiting finetune (downstream) using ugf1f2ld pretrain checkpoints.\n"
        "# Auto-updated by scripts/generate_fruiting_sweep_yaml.py\n"
        "#\n"
        "# Usage:\n"
        "#   source setup.sh\n"
    ...
    print(f"Updated {SWEEP_YAML} with {len(run_ids)} pretrained_run_id values.")
...
def main() -> None:
    ...
    parser.add_argument("--sweep-id", default=PRETRAIN_SWEEP_ID)
    parser.add_argument("--top-k", type=int, default=12)
    parser.add_argument(
        "--include-missing-weights",
        action="store_true",
        help="Include runs even if ~/weights/.../sweep_<id>.pth is absent",
    )
    parser.add_argument("--list", action="store_true", help="Print pretrain runs and exit")
    parser.add_argument("--write", action="store_true", help="Write top-k run ids into sweep yaml")
    ...
    if not runs:
        raise SystemExit("No finished pretrain runs with local weights found.")
