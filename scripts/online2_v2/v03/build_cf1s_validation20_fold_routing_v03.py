#!/usr/bin/env python3
"""Build CF1S_VALIDATION20_FOLD_ROUTING_V1.json: 20 finetune-unseen,
selection-blind-to-Fold2 cases, same search/reevaluation fold structure as
Development3 (search=[0,1], reevaluation=[2]), training_seen=false."""
...
def main() -> int:
    ...
    if len(case_ids) != 20:
        raise CoreContractError("Validation20 requires exactly 20 Problem20 cases")

    routing = {
        "manifest_id": "CF1S_VALIDATION20_FOLD_ROUTING_V1",
        "cohort": "validation20",
    ...
    out = ROOT / "conf/m1/cf1s_policies/cohorts/CF1S_VALIDATION20_FOLD_ROUTING_V1.json"
    if out.exists():
